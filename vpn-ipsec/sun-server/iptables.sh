...
do_start() {
    ...
    ip tunnel add vti0 local 10.10.0.20 remote 10.10.0.10 mode vti key 42
    ip link set vti0 up

    ip addr add 10.1.1.1/32 dev vti0
    ip route add 10.1.1.2/32 dev vti0

    ip route add 10.10.2.0/24 dev vti0
    ...
    iptables -w 3 -t nat -A POSTROUTING -s 10.1.1.2/32 -d 10.10.2.0/24 -j SNAT --to-source 10.10.3.2

}

do_stop() {

    iptables -t nat -D POSTROUTING -s 10.1.1.2/32 -d 10.10.2.0/24 -j SNAT --to-source 10.10.3.2

    iptables -t nat -D POSTROUTING --destination 10.10.2.0/24 -j ACCEPT
    iptables -t nat -D POSTROUTING --source 10.10.2.0/24 -j ACCEPT
    
    ip route del 10.10.2.0/24 dev vti0

    ip route del 10.1.1.2/32 dev vti0
    ip addr del 10.1.1.1/32 dev vti0
